...
const Main = __webpack_require__(9);
...
var fa = __webpack_require__(10);
var gi = __webpack_require__(11);

var List = __webpack_require__(12);
var RadioSelector = __webpack_require__(13);
...
var Main = function (_React$Component) {
	...
	_createClass(Main, [{
		key: 'changeSet',
		value: function changeSet(value) {
			this.setState({
				iconSet: value
			});
		}
		...
		value: function render() {
			var self = this;
			var set = self.state.iconSet === 'fa' ? fa : gi;
			document.title = 'Icons ' + (self.state.iconSet === 'fa' ? 'FA' : 'GI');
			return React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						RadioSelector,
						{ id: 'set', name: 'set', onChange: self.changeSet.bind(self), value: self.state.iconSet },
						React.createElement(
							'option',
							{ value: 'fa' },
							'FA'
					...
					React.createElement('input', { name: 'search', value: self.state.searchText, onChange: self.search.bind(self), placeholder: 'Search' })
				),
				React.createElement(List, { set: set, iconSet: self.state.iconSet, searchText: self.state.searchText })
			);
		}
	}]);
	...
	_createClass(List, [{
		...
		value: function render() {
			...
			return React.createElement(
				"div",
				null,
				props.set.map(function (elt) {
					return elt.indexOf(props.searchText) > -1 ? React.createElement(
						"div",
						{ "class": "glyph", key: elt },
						React.createElement("span", { "class": props.iconSet + " " + props.iconSet + "-" + elt, "aria-hidden": "true" }),
						React.createElement(
							"label",
							null,
							elt
...
var Option = function Option(elt) {
	return React.createElement(
		"a",
		{ key: elt.value,
...
var RadioSelector = function (_React$Component) {
	_inherits(RadioSelector, _React$Component);

	function RadioSelector(props) {
		_classCallCheck(this, RadioSelector);

		var _this = _possibleConstructorReturn(this, (RadioSelector.__proto__ || Object.getPrototypeOf(RadioSelector)).call(this));
			...
			if (Array.isArray(props.children)) {
				for (var i = 0; i < props.children.length; i++) {
					if ('option' === props.children[i].type) {
						_this.options.push(_this.childrenToOptions(props.children[i].props));
					} else {
						var eltName = '';
				...
				_this.options.push(_this.childrenToOptions(props.children.props));
			}
		}
	...
	_createClass(RadioSelector, [{
		...
		value: function childrenToOptions(props) {
			return {
				value: props.value,
		...
		value: function click(event) {
			...
			if (this.props.onChange) {
				this.props.onChange(event.target.attributes.value.value);
			}
		}
	}, {
		key: "render",
		value: function render() {
			var self = this;
			var containerClassName = 'radio-selector' + (self.props.className ? ' ' + self.props.className : '');

			return React.createElement(
				"div",
				{ className: containerClassName },
				React.createElement("input", { type: "hidden", name: self.props.name, value: self.state.value, id: self.props.id }),
				self.options.map(function (elt) {
